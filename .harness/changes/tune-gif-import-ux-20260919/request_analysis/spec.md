...
## 7. 方案修订 v2（2026-09-19，真机验证反馈驱动）

**反馈**：真机/PC 上"无论发多少 GIF 到聊天都是无结果"。判定：用户以图片气泡形式发送（相册直发/PC 拖拽/转发动图均为图片消息），type:'file' + extension:['gif'] 完全不可见。

**修订**（推翻第 2 节"非目标"第一条，理由：验证期实测证据优先于预判）：

- 主入口 `chooseMessageFile` 由 `type:'file' + extension:['gif']` 放宽为 **`type:'all'`（去掉 extension）**——图片气泡/文件卡片形式的 GIF 都出现在可选列表，彻底消除"无结果"困惑。
- GIF8 头部校验照旧兜底：图片消息若被微信转静态图 → 分来源报错引导"以「文件」发送最稳"。
- 指引弹窗文案同步：第 ① 步改为"发到任意聊天"，"最稳：+ → 文件"，保留 iOS 相册提示。
- 相册辅助入口不变。

**关键待验事实（决定 v2 去留）**：真机上以图片气泡发送的 GIF 经 type:'all' 选中后，字节是否保持 GIF 动图（社区证据倾向：动图在聊天内保持动画，底层存的就是 .gif 原文件）。
- 若保持 → v2 是终态，用户无需"以文件发送"这步，体验最优；
- 若被转静态 → 保留 v2（列表至少可见），报错引导到"以文件发送"，等效 v1 + 可见性。两分支都比 v1 好，无回退必要。
