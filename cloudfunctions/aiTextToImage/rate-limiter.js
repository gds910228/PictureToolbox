...
//     · featureKey 区分功能（outpaint / caption / avatar / text2img ...），
//       各功能计数独立、互不共享额度。
...
//   const rl = await rateLimiter.checkRateLimit(openid, 'outpaint', cloud);
...
//
//   const q = await rateLimiter.queryQuota(openid, 'outpaint', cloud);  // 只读，不计数
 ...
 * @param {string} featureKey 功能标识（如 'outpaint'）
